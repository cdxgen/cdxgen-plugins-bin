...
| `dispatch_trait`          | `string?` | Trait whose vtable dispatched this call, when the receiver is a trait object and the target resolved through that trait's impls. A trait object carries exactly one vtable — the principal trait's — so this names the closed set of impls the call can reach. Omitted for every other resolution shape, including blanket-impl dispatch, where the receiver selects no single trait. Both backends emit it, and both emit the bare trait name so the field joins across them: the stable backend reduces the written receiver type, the compiler backend takes the trait owning the called method from `TyCtxt`. One asymmetry remains. The compiler backend tests the receiver for `ty::Dynamic` after peeling references only, so a `Box<dyn T>` or `Arc<dyn T>` receiver is classified `trait-static` rather than `dyn-dispatch` and carries no trait, while the stable backend names it. A bare `&dyn T` receiver is named by both. |
...
> **A trait-object receiver dispatches on its principal trait.** `Box<dyn Error
> + Send + Sync>` carries one vtable, `Error`'s — the bounds after `+` are auto
> traits and lifetimes and add no vtable entries — so it resolves exactly like
> `dyn Error`, and the edges record `dispatch_trait: "Error"`. Collections of
> trait objects dispatch on their element: iterating or indexing a
> `Vec<Box<dyn Handler>>` types the receiver as `Handler`, while the
> container's own methods stay container-typed (`push` is `Vec::push`).
...
| Field          | Type               | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| -------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `kind`         | `string`           | Diagnostic class. Common values: `parse`, `resolution`, `backend`, `missing-passthrough`, `module-resolution`, `import-resolution`. `missing-passthrough` diagnostics indicate method calls where taint tracking was lost; the message suggests adding the method as a passthrough pattern. `module-resolution` reports a `mod` declaration that could not be resolved, or a file no crate root reaches (analyzed with a module path derived from its file path). `import-resolution` reports a glob import whose target module is outside the workspace, so names imported through it stay unresolved. `macro` reports a macro whose body could not be recovered as Rust (including every `macro_rules!` definition, which is deliberately not walked), naming the files it appears in so the blind spot is visible. `object-safety` (in `call_graph.diagnostics`) reports a `dyn` receiver naming a local trait that reads as not object safe — a `-> Self` return, a generic method, or by-value `self`, none excused by `where Self: Sized`. Such a trait cannot back a vtable, so **no dispatch edges were emitted for that call**; the message names the trait and every reason. The reader is a syntactic approximation of rustc's dyn-compatibility rules and is biased to under-report, since a false reading deletes real edges: an associated-item return (`-> Option<Self::Item>`, the `Iterator` shape) and a type merely *named* like `Self` are both dyn compatible and are not gated. |
