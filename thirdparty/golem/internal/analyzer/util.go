...
var generatedByPattern = regexp.MustCompile(`(?i)^// Code generated by\s+([^;.]+)`) // keep only the generator token, not raw command text.
...
func generatedInfo(path string) (bool, string) {
	...
	for i := 0; scanner.Scan() && i < 40; i++ {
		...
		matches := generatedByPattern.FindStringSubmatch(line)
		if len(matches) > 1 {
