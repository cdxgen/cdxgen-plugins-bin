...
| `runtime.goExperiment`                                   | string, optional           | Effective `GOEXPERIMENT` the load ran under, resolved with `go env` in the load environment (for example `simd`). | Know whether experiment-gated packages (`simd`) were part of the analysis. |
| `runtime.targetGoos`                                     | string, optional           | Effective `GOOS` the packages were loaded for, from the same `go env` call. Unlike `runtime.goos` — golem's own platform — this names the load target and differs when `GOOS` was set for the analysis. | Cross-architecture analysis review; explaining arch-specific file inclusion. |
| `runtime.targetGoarch`                                   | string, optional           | Effective `GOARCH` the packages were loaded for; same relationship to `runtime.goarch` as `targetGoos` has to `goos`. | Confirming which architecture's code was actually analysed (e.g. amd64-only SIMD). |
