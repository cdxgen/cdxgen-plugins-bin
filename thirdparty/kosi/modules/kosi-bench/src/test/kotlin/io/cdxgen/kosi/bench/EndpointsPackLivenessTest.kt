...
class EndpointsPackLivenessTest {
    ...
    private fun inertAllowance(): Map<String, String> = mapOf(
        ...
        "ratpack.contextReaders[ratpack.http.Request.getHeaders]" to
            "P30: a ratpack contextReader whose kind is already carried by ANOTHER reader on the same handler set — this sweep diffs Endpoints.Result, where the endpoint records the parameter KIND, so the second row of a kind it already has adds nothing visible to it. The gate that does see it is fixtures/ratpack-dropwizard, which has one `kosi:want flow` per reader (ratpack.http.Request.getHeaders among them) and the security-pack source row behind it: remove the row and that want fails by name. Both package generations are exercised, 1.x and 2.x, because R168 is the defect where a framework named in one spelling is invisible in the other. Transcribed from ratpack.io/manual/current/api/ratpack/core/http/Request.html, not from memory.",
        "ratpack.contextReaders[ratpack.http.Request.getCookies]" to
            "P30: a ratpack contextReader whose kind is already carried by ANOTHER reader on the same handler set — this sweep diffs Endpoints.Result, where the endpoint records the parameter KIND, so the second row of a kind it already has adds nothing visible to it. The gate that does see it is fixtures/ratpack-dropwizard, which has one `kosi:want flow` per reader (ratpack.http.Request.getCookies among them) and the security-pack source row behind it: remove the row and that want fails by name. Both package generations are exercised, 1.x and 2.x, because R168 is the defect where a framework named in one spelling is invisible in the other. Transcribed from ratpack.io/manual/current/api/ratpack/core/http/Request.html, not from memory.",
        "ratpack.contextReaders[ratpack.http.Request.oneCookie]" to
            "P30: a ratpack contextReader whose kind is already carried by ANOTHER reader on the same handler set — this sweep diffs Endpoints.Result, where the endpoint records the parameter KIND, so the second row of a kind it already has adds nothing visible to it. The gate that does see it is fixtures/ratpack-dropwizard, which has one `kosi:want flow` per reader (ratpack.http.Request.oneCookie among them) and the security-pack source row behind it: remove the row and that want fails by name. Both package generations are exercised, 1.x and 2.x, because R168 is the defect where a framework named in one spelling is invisible in the other. Transcribed from ratpack.io/manual/current/api/ratpack/core/http/Request.html, not from memory.",
        "ratpack.contextReaders[ratpack.http.Request.getBody]" to
            "P30: a ratpack contextReader whose kind is already carried by ANOTHER reader on the same handler set — this sweep diffs Endpoints.Result, where the endpoint records the parameter KIND, so the second row of a kind it already has adds nothing visible to it. The gate that does see it is fixtures/ratpack-dropwizard, which has one `kosi:want flow` per reader (ratpack.http.Request.getBody among them) and the security-pack source row behind it: remove the row and that want fails by name. Both package generations are exercised, 1.x and 2.x, because R168 is the defect where a framework named in one spelling is invisible in the other. Transcribed from ratpack.io/manual/current/api/ratpack/core/http/Request.html, not from memory.",
        "ratpack.contextReaders[ratpack.http.Request.getBodyStream]" to
            "P30: a ratpack contextReader whose kind is already carried by ANOTHER reader on the same handler set — this sweep diffs Endpoints.Result, where the endpoint records the parameter KIND, so the second row of a kind it already has adds nothing visible to it. The gate that does see it is fixtures/ratpack-dropwizard, which has one `kosi:want flow` per reader (ratpack.http.Request.getBodyStream among them) and the security-pack source row behind it: remove the row and that want fails by name. Both package generations are exercised, 1.x and 2.x, because R168 is the defect where a framework named in one spelling is invisible in the other. Transcribed from ratpack.io/manual/current/api/ratpack/core/http/Request.html, not from memory.",
        "ratpack.contextReaders[ratpack.core.http.Request.getHeaders]" to
            "P30: a ratpack contextReader whose kind is already carried by ANOTHER reader on the same handler set — this sweep diffs Endpoints.Result, where the endpoint records the parameter KIND, so the second row of a kind it already has adds nothing visible to it. The gate that does see it is fixtures/ratpack-dropwizard, which has one `kosi:want flow` per reader (ratpack.core.http.Request.getHeaders among them) and the security-pack source row behind it: remove the row and that want fails by name. Both package generations are exercised, 1.x and 2.x, because R168 is the defect where a framework named in one spelling is invisible in the other. Transcribed from ratpack.io/manual/current/api/ratpack/core/http/Request.html, not from memory.",
        "ratpack.contextReaders[ratpack.core.http.Request.getCookies]" to
            "P30: a ratpack contextReader whose kind is already carried by ANOTHER reader on the same handler set — this sweep diffs Endpoints.Result, where the endpoint records the parameter KIND, so the second row of a kind it already has adds nothing visible to it. The gate that does see it is fixtures/ratpack-dropwizard, which has one `kosi:want flow` per reader (ratpack.core.http.Request.getCookies among them) and the security-pack source row behind it: remove the row and that want fails by name. Both package generations are exercised, 1.x and 2.x, because R168 is the defect where a framework named in one spelling is invisible in the other. Transcribed from ratpack.io/manual/current/api/ratpack/core/http/Request.html, not from memory.",
        "ratpack.contextReaders[ratpack.core.http.Request.oneCookie]" to
            "P30: a ratpack contextReader whose kind is already carried by ANOTHER reader on the same handler set — this sweep diffs Endpoints.Result, where the endpoint records the parameter KIND, so the second row of a kind it already has adds nothing visible to it. The gate that does see it is fixtures/ratpack-dropwizard, which has one `kosi:want flow` per reader (ratpack.core.http.Request.oneCookie among them) and the security-pack source row behind it: remove the row and that want fails by name. Both package generations are exercised, 1.x and 2.x, because R168 is the defect where a framework named in one spelling is invisible in the other. Transcribed from ratpack.io/manual/current/api/ratpack/core/http/Request.html, not from memory.",
        "ratpack.contextReaders[ratpack.core.http.Request.getBody]" to
            "P30: a ratpack contextReader whose kind is already carried by ANOTHER reader on the same handler set — this sweep diffs Endpoints.Result, where the endpoint records the parameter KIND, so the second row of a kind it already has adds nothing visible to it. The gate that does see it is fixtures/ratpack-dropwizard, which has one `kosi:want flow` per reader (ratpack.core.http.Request.getBody among them) and the security-pack source row behind it: remove the row and that want fails by name. Both package generations are exercised, 1.x and 2.x, because R168 is the defect where a framework named in one spelling is invisible in the other. Transcribed from ratpack.io/manual/current/api/ratpack/core/http/Request.html, not from memory.",
        "ratpack.contextReaders[ratpack.core.http.Request.getBodyStream]" to
            "P30: a ratpack contextReader whose kind is already carried by ANOTHER reader on the same handler set — this sweep diffs Endpoints.Result, where the endpoint records the parameter KIND, so the second row of a kind it already has adds nothing visible to it. The gate that does see it is fixtures/ratpack-dropwizard, which has one `kosi:want flow` per reader (ratpack.core.http.Request.getBodyStream among them) and the security-pack source row behind it: remove the row and that want fails by name. Both package generations are exercised, 1.x and 2.x, because R168 is the defect where a framework named in one spelling is invisible in the other. Transcribed from ratpack.io/manual/current/api/ratpack/core/http/Request.html, not from memory.",
        "ratpack.contextParameterTypes[ratpack.handling.Context]" to
            "P30: consumed by the FLOW ENGINE's parameter seeding, not by endpoint detection, so this sweep — which diffs Endpoints.Result — structurally cannot see it. The gate that does is fixtures/ratpack-dropwizard's ContextOnlySink and LegacyContextOnlySink: the context is the framework's own collaborator, and if it stopped being one those handlers would report a flow their want-nots forbid. ratpack.io/manual/current/handlers.html: `void handle(Context context)`.",
        "ratpack.contextParameterTypes[ratpack.core.handling.Context]" to
            "P30: consumed by the FLOW ENGINE's parameter seeding, not by endpoint detection, so this sweep — which diffs Endpoints.Result — structurally cannot see it. The gate that does is fixtures/ratpack-dropwizard's ContextOnlySink and LegacyContextOnlySink: the context is the framework's own collaborator, and if it stopped being one those handlers would report a flow their want-nots forbid. ratpack.io/manual/current/handlers.html: `void handle(Context context)`.",
        "quarkus.nonInputAnnotations[io.dropwizard.auth.Auth]" to
            "P30: consumed by the FLOW ENGINE's parameter seeding, not by endpoint detection, so this sweep — which diffs Endpoints.Result — structurally cannot see it. The gate that does is fixtures/ratpack-dropwizard's `whoami` want-not, RESTORE-PROVEN: remove this row and whoami reports an untrusted-input flow from a principal the framework injected. dropwizard.io/en/stable/manual/auth.html: \"annotate the parameter representing your principal with @Auth\".",
