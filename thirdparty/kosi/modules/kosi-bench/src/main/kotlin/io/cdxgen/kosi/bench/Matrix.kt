...
/**
 * The bench matrix (golem's manifest.go shape). There is one backend
...
object Matrix {
    ...
    /**
     * / 10-DEEP-EVIDENCE.md §3: the deep tier runs the DEFAULT slot
    ...
    /**
     * The matrix runs every case three ways: the two syntax slots
     * (dataflow modes, as) and the resolved backend, so the resolved
     * tier's expectations are ratcheted per fixture exactly like the syntax
     * tier's. Adds the `exported` slot: the resolved backend rooted at the
     * public API (`--roots exported`), because reachability from `main` alone
     * yields nothing on a library — golem's lesson — and the edge gates
    ...
    fun defaultMatrix(): List<MatrixSlot> = listOf(
        ...
        // DELETED the `all` slot. It ran the SYNTAX backend, which
        ...
        // its own flag (a slot nobody's analysis differs on proves
        ...
        // Endpoint-rooted taint. The resolved backend with handler
        ...
        // The dependency tier. The resolved backend plus --deps: classpath
        ...
        // Endpoint parameters seed entry-point facts: at repo scale the
