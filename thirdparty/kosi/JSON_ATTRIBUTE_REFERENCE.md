...
| `invokedArgSinks[]` | string[] | sinks the body reaches with an argument it handed to an invoked function-valued parameter, as `p<j>(arg<k>).<path>` — the DSL builder's channel: `b.block(); b.go()` inside the callee sinks the very object it passed as argument k of the lambda, so a lambda that writes its capture into that argument's field (the write is the lambda's own `paramFieldWrites`) reaches that sink |
...
- `dsl`: a routing call, with the handler resolved to the extracted lambda
  ...
  - registrations in code, such as Spring Boot's `ServletRegistrationBean`
    and `FilterRegistrationBean` and the WebSocket and STOMP handshake
    registries;
